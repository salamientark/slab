...
Reproducible install across machines. Vendored configs in `dist/`, idempotent `install.sh`, clean `uninstall.sh`.
...
- **Marker blocks.** Appended config uses `# >>> i3-notch >>>` / `# <<< i3-notch <<<` sentinels. Installer and uninstaller identify managed sections by markers only — never free-form edits.
...
├── uninstall.sh                  # revert (markers + backups)
...
1. `systemctl --user disable --now i3-notch` then remove unit file.
2. Strip marker block from `~/.config/polybar/config.ini` and `launch.sh`.
3. Restore `~/.claude/settings.json` from most recent `.bak-*` (or strip hooks block by marker if merged in-place).
4. Remove symlinks from `~/.local/bin/`.
5. Leave the repo clone in place (user's choice to `rm -rf`).
...
- Uninstall = delete block via `sed -i '/# >>> i3-notch >>>/,/# <<< i3-notch <<</d'`.
...
5. Run `uninstall.sh`. Expect `config.ini` byte-identical to pre-install backup.
