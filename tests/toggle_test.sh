...
case_start_falls_back_to_nohup_when_raise_unit_missing() {
  setup_stubs
  # Override systemctl: list-unit-files exits 1 (no raiser unit); start ok.
  cat > "$STUBDIR/systemctl" <<EOF
#!/usr/bin/env bash
printf '%s %s\n' "systemctl" "\$*" >> "$LOG"
case "\$*" in
...
EOF
  ...
  STUB_SYSTEMCTL_ACTIVE=1 STUB_PGREP_RC=1 \
    "$TOGGLE" start >/dev/null 2>&1
  local rc=$?
  if grep -qE 'systemctl --user start +i3-notch-raise' "$LOG"; then
    teardown_stubs; return 1
  fi
  grep -q '^nohup ' "$LOG" || { teardown_stubs; return 1; }
  teardown_stubs
  [ "$rc" -eq 0 ]
}
...
run_case "start falls back to nohup when raise unit missing" case_start_falls_back_to_nohup_when_raise_unit_missing
